...
class ImageService {
	...
	public  static boolean SaveImageonFolder(byte[]  srcFile, String nameImage, int idUser ){
		BufferedImage src = ImageIO.read(new ByteArrayInputStream(srcFile));
		File outputfile = new File("web-apps/images/"+idUser.toString()+"nameImage.png");
		if (ImageIO.write(src, "png", outputfile)){
			return true ;
		}
		 return false;
